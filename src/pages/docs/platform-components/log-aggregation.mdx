...
[Gigapipe](https://gigapipe.com/index.html) is a Loki compatible log (and metrics) aggregation system using [ClickHouse](https://clickhouse.com/) under the hood. [Vector](https://vector.dev/) is used to feed logs into `gigapipe`. Finally we configure Grafana from our [Monitoring](/platform-components/monitoring) stack to use `gigapipe` as a data source.
...
To feed gigapipe with logs we previously used Promtail. After its deprecation we chose Vector over Alloy due to having better performance and a smaller footprint.
