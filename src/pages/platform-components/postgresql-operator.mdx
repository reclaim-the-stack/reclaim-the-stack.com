...
In your DevOps repository, run `k generate resource <application-name> postgres` to generate a new database resource. This will add a `PostgresCluster` template under `applications/<application-name>/templates` and output some Helm configurationn for you to manually add into `applications/<application-name>/values.yaml`.
